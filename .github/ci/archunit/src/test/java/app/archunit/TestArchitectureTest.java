 ...
 * not apply here — unit-test paths mirror the app source, so a test inherits segment names that
 * mean something different for a test than for the class it covers. These rules are the Java
 * spelling of the PHPArkitect test rules the reference implementation enforces.
 *
 * <p>The classes are imported by path rather than scanned off the classpath so the two rule sets
 * stay strictly separated: the app source is checked by {@link ArchitectureTest}, the test tree
 * only here.
 */
